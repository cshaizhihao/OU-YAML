...
网页中的“内核实测”不会下载或执行用户提供的程序，只会调用服务器管理员预先安装的 Mihomo 或 sing-box 二进制。Docker 部署时可参考 `docker-compose.kernel.example.yml`，确认宿主机路径后运行：

```bash
docker compose -f docker-compose.yml -f docker-compose.kernel.example.yml up -d --build
```

未挂载相应内核时，编辑、结构校验和导出功能仍可正常使用，界面会明确提示内核不可用。
